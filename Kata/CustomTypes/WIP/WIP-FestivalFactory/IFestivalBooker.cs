...
a music promoter needs to process applications from musical acts to fill a festival itinerary.  each venue caters to a specific musical taste.  

some of the venues have more than one performance space, and all have different capacities and times between which they can put acts on.
...
define an overload for AddAct with a single parameter, that is an anonymous method with no return value (i.e. an Action) 

Call this parameter getSignedActInfo and have it accept an ActBase as a parameter
...
Define a method PrintItineray that takes a list of VenueBase as a parameter returns void.
...
Use ActBase as the abstract creator for a sub-factory pattern with a new abstract product SongBase.
In ActBase called the list of SongBase "Repertoire".
...
namespace Kata.CustomTypes.FestivalFactoryDone
{
    ...
    {
        FestivalApplication Apply(ActBase act, string VenueName, string prefSpace, TimeSpan prefSlot, bool standby);
        VenueBase GetVenueByAct(ActBase act);
        void Notify(FestivalApplication appl, NotificationType notification);
        void Accept(FestivalApplication appl);
        void Decline(FestivalApplication appl);
